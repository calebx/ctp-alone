...
fn main() {
    ...
    std::fs::copy(&format!("{}/../ctp-api/{}", current_dir, SO_FILENAME), &target_so).expect("failed to copy so to outdir");
    ...
    std::os::unix::fs::symlink(target_so, so_symlink).expect("failed to create new symlink");
